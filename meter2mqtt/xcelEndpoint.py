...
class xcelEndpoint():
    ...
    def create_config(self, sensor_name: str,  details: dict) -> tuple[str, dict]:
        ...
        payload["state_topic"] = f'homeassistant/sensor/Instantaneous_Demand/value/state'
    ...
    def process_send_mqtt(self, reading: dict) -> None:
        ...
        Returns: None
        """
        # Send to Home Assistant topics (existing functionality)
        mqtt_topic_message = {}
        ...
        for k, v in reading.items():
            ...
            sFDI = self.device_info['device']['identifiers'][0]
            payload_with_device_info = {
                'sFDI': sFDI,
                'value': v,
                'unit': 'watt'
            }
            ...
            print(f"Full payload: {json.dumps(payload_with_device_info)}")
            
            # Create dict of {topic: payload} - publish only the value
            mqtt_topic_message[topic] = str(v)
    ...
    def mqtt_publish(self, topic: str, message: str, retain=False) -> int:
        ...
        # Check if client is connected and try to reconnect if needed
        if not self.client.is_connected():
            print("WARNING: MQTT client is not connected! Attempting to reconnect...")
            try:
        ...
        if result[0] == mqtt.MQTT_ERR_SUCCESS:
            print(f"MQTT Send Result: \t\tSUCCESS (rc={result[0]})")
        else:
            print(f"MQTT Send Result: \t\tFAILED (rc={result[0]})")
